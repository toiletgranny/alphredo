...
const settings = {
	...
	render: () => {
		...
		if (settings.background === 'white' || settings.background === 'black') {
			document.querySelector(`input[name="background-mode"][value="${settings.background}"]`).checked = true;
		...
		document.querySelector(`input[name="saturation-mode"][value="${settings.saturation.mode}"]`).checked = true;
		document.querySelector('#saturation-adjustments').style.display = settings.saturation.mode === "custom" ? "inherit" : "none"
		document.querySelector(`option[value="${settings.saturation.easing}"]`).selected = true;
		document.querySelector('input[name="saturation-intensity"]').value = settings.saturation.intensity;
		document.querySelector('input[name="saturation-offset"]').value = settings.saturation.offset;
	},
	update: () => {
		document.querySelector(settings.selectors.setinngs).addEventListener("input", () => {
			const { user: { settings } } = palette;
